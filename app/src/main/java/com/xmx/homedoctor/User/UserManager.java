...
public class UserManager {
    ...
    public void logout() {
        if (isLoggedIn()) {
            ...
            AVQuery<AVObject> query = new AVQuery<>("DoctorsData");
    ...
    public void register(final String username, final String password, final String nickname, final RegisterCallback registerCallback) {
        final AVQuery<AVObject> query = AVQuery.getQuery("DoctorsInf");
        query.whereEqualTo("username", username);
            ...
            public void done(final int count, AVException e) {
                if (e == null) {
                    if (count > 0) {
                        ...
                        AVQuery<AVObject> query2 = AVQuery.getQuery("DoctorsData");
                            ...
                            public void done(int i, AVException e) {
                                if (e == null) {
                                    if (i > 0) {
                                        ...
                                        final AVObject post = new AVObject("DoctorsInf");
                                        post.put("username", username);
                                        post.put("password", UserManager.getSHA(password));
                                        post.put("status", 0);
                                        post.put("timestamp", System.currentTimeMillis() / 1000);

                                        final AVObject data = new AVObject("DoctorsData");
    ...
    public void login(final String username, final String password, final LoginCallback loginCallback) {
        AVQuery<AVObject> query = new AVQuery<>("DoctorsInf");
        query.whereEqualTo("username", username);
        query.findInBackground(new FindCallback<AVObject>() {
            @Override
